...
- **SVG icon fallback rule**: this only applies when you drop to an inline `<svg>` because no glyph class fits. In that case, the `<svg>` element MUST have `class="icon"` directly on it plus explicit `width` and `height` attributes (e.g. `width="16" height="16"`). Never put `class="icon"` on a wrapping `<span>`. Glyphs (the preferred path) are plain `<span class="glyph glyph-*"></span>` and do NOT use `class="icon"`.
