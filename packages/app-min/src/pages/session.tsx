...
export default function Page() {
  ...
  const report = (cedula: string) => {
    ...
    return [
      ...
      // — Regla de orden estricto (ARRIBA DE TODO, antes que cualquier PASO) —
      "REGLA DE ORDEN ESTRICTO (LEE ESTO ANTES DE CUALQUIER PASO): Los 5 PASOS son SECUENCIALES. Cada PASO debe COMPLETARSE por completo y recibir confirmacion de exito antes de pasar al siguiente. PROHIBIDO ejecutar varios PASOS en paralelo en el mismo turno. PROHIBIDO llamar a la tool 'pdf' en el mismo turno (o lote de tool-calls paralelos) que la tool 'Write' — primero Write debe devolver confirmacion de exito, y SOLO DESPUES, en una llamada posterior, llamas a 'pdf'. Llamar a 'pdf' antes de que Write termine produce el error 'html source not found' porque el archivo HTML aun no existe. Paralelizar NO es mas rapido: rompe el flujo y el PDF falla. Haz los pasos uno a uno.",

      "PASO 1 — CARGAR LA SKILL (OBLIGATORIO, cada vez): Activa y consulta la skill PDF (`pdf`) ANTES de cualquier otra accion, sin importar si la conversacion es nueva o existente. La skill contiene las reglas de margenes, tipografia, paleta, tablas, charts SVG, glyphs, densidad de pagina y footer — es la fuente de verdad del diseno. Si la skill no esta cargada en contexto, usa la tool Read sobre `.opencode/skills/pdf/SKILL.md` y `.opencode/skills/pdf/STARTER.html` relativo al project root. PROHIBIDO asumir que ya la conoces de una sesion anterior. PROHIBIDO improvisar el diseno sin consultar la skill. Recien despues de tener la skill a la vista puedes pasar al PASO 2.",
      ...
      "PASO 3 — ESCRIBIR EL HTML (siguiendo la skill al pie de la letra): Con todos los datos recopilados, compone el HTML completo siguiendo la skill PDF y escribe el archivo usando la tool Write. El HTML debe cubrir TODOS los datos obtenidos del MCP, organizados en las capas del reporte. Un reporte completo ocupa 4-8 paginas o mas. No resumas ni descartes datos. PROHIBIDO declarar bloques @page de cualquier tipo en el HTML — el renderer ya controla tamaño A4, margenes 24/16/22/16 mm y numeracion de pagina. PROHIBIDO poner margin, padding, width, max-width o min-width en html o body. Incluir `@page { margin: 0 }` o `body { margin: 0; width: 100% }` rompe el centrado y produce un reporte con margen izquierdo chico y margen derecho enorme. OBLIGATORIO envolver TODO el contenido en <main class=\"doc\"> para que la columna de 171mm quede centrada. OBLIGATORIO aplicar el Chart Pre-Write Checklist de la skill antes de emitir cualquier SVG de grafica (verifica numero de <rect>, altura proporcional, rx <= 6, nunca rx=999, nunca <line> como barra, nunca div con width% como chart). OBLIGATORIO colocar la .footer-note como ultimo hijo del ultimo .sheet, nunca en su propio .sheet. OBLIGATORIO agrupar 2-4 secciones sustanciales por .sheet; una seccion corta NUNCA va sola en su propio .sheet.",
      ...
      "CAPA 3 — Capa de detalle: secciones H2 una por area tematica, cada una con parrafo interpretativo de 2-4 oraciones mas un elemento visual (tabla, grafica, callout o banda). Si los datos incluyen valores numericos, comparativos o series de tiempo, DEBE incluir una grafica usando la libreria SVG de charts de la skill; usa SVG manual solo como respaldo cuando la libreria no aplique.",
